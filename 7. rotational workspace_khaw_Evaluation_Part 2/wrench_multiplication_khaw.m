function [wrench] = wrench_multiplication_khaw (w_p, w_p_t, lever_arm)
...
rot_angle_wp = wp_rot_angles_3Daxis(counter_3Daxis); 
    ...
    for counter_angles_z = 1: length(wp_angles_z) %rotate at xy-axis (every 45°) to 360° 
        ...
        sphere_mat(:,counter) = sphere_vec;
    end
end

sphere_mat(:,end)=[0 0 1]; %for the very top point,so not 8 times repetition at the same point
sphere_mat(:,end-1)=[0 0 -1]; %for the very bottom point,so not 8 times repetition at the same point 

wrench_f= sphere_mat* w_p; %external force in N
wrench_t= sphere_mat* w_p_t* -lever_arm; %external torque in Nmm
