...
function [workspace_logical, R] = Arbeitsraum_khaw(a, b, f_min, f_max,noC, rotation, w_p_x, w_p_t, rotation_w_p_x,rotation_w_p_y, workspace_logical, pulley_kin, rad_pulley, R_A, rot_angle_A, coordinate, limit, f_direction)
...
rotation_matrix.wpx = axang2rotm(rotation_w_p_x); %rotation matrix for f_Y around X-AXIS (IMPORTANT)
rotation_matrix.wpy = axang2rotm(rotation_w_p_y); %rotation matrix for f_X around Y-AXIS (IMPORTANT)
...
%Go through all the coordinate combination of the x_row, y_column, z_page, and save them in variable workspace_position 
       for i = 1:length(coordinate.x)
        ...
        [stop, R ] = berechnungSeilkraftverteilung_KHAW(workspace_position, a, b, f_min, f_max,noC, rotation, w_p_x, w_p_t, rotation_matrix, pulley_kin, rad_pulley, R_A, rot_angle_A, limit, f_direction); %hier erstmal nur stop von Interesse tbd
