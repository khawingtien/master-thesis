       ...
       for i = 1:length(coordinate.x)
         for j = 1:length(coordinate.y)
           for k = 1:length(coordinate.z)
             workspace_position = [coordinate.x(i) coordinate.y(j) coordinate.z(k)-offset]'; %workspace_position in a column vector 
%              workspace_position =  workspace_position - offset;    
