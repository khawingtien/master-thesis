...
% offset.y=0;
% offset.z= b(3,5);
% coordinate.z = coordinate.z - b(3,5);
poi=[0 0 b(3,5)];
rotation_array_values = 0:1:45; %13 times rotation angle
%  rotation_array_values = :0;
z_values=poi(3):10:-poi(3);
...
%            rotation_Angle = rotation_array_values(i);
%            rotation_cell=[0 1 0 ((pi/180) * rotation_Angle)]; %rotation at z-axis with rotation angle in radian
%              R = axang2rotm(rotation_cell);
             %workspace_position = [coordinate.x(i) coordinate.y(j) coordinate.z(k)]'; %workspace_position in a column vector 
             %workspace_position = R*poi';
             workspace_position=[0 0 z_values(i)]';
        ...
        %write the value (True or false) into the workspace logical matrix  
            if stop == 0  %no violation of f_min & f_max (fulfil the requirements)(TRUE)
              workspace_logical_temp (i) = 1; %write 1 as TRUE
            elseif stop == 1 %if violation exist 
              workspace_logical_temp (i) = 0; %write 0 as FALSE 
            end
       end
